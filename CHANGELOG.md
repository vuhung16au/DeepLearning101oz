...
### Added
- Problem numbering policy in GitHub instruction files (`.github/copilot-instructions.md` and `.github/instructions/gemini.instructions.md`)
  - Documented automatic numbering system: "Problem X.Y" where X is chapter number and Y is sequential number
  - Clarified that problem counter is shared with theorems, definitions, and examples via `\newtheorem{problem}[theorem]{Problem}`
  - Added detailed problem structure guidelines with LaTeX code examples
  - Specified requirement for descriptive problem titles in square brackets
  - Documented difficulty organisation using unnumbered subsections (Easy/Medium/Hard)
  - Added "Problem Numbering and Format" section explaining the numbering mechanism
- Problem creation workflow in Common Tasks/Development Workflow sections
  - Step-by-step guide for creating problem files (`chapters/chapXX-problems.tex`)
  - Template with section headers and difficulty subsections
  - Integration instructions for including problems in chapter files
- Problem Set Template in Gemini instructions showing complete example structure
...
- **Date**: 16 Oct 2025
- **License**: Consistent with project license
- **Source**: `.github/copilot-instructions.md`, `.github/instructions/gemini.instructions.md`
- **Destination**: AI assistant instruction files for GitHub Copilot and Google Gemini
- **Dependencies**: No new dependencies
- **Instructions**: Changes are documentation-only, no build process modifications required
- **Notes**: 
  - Problem numbering uses existing LaTeX theorem environment defined in main.tex (line 148)
  - Numbering format "Problem X.Y" is automatically generated by LaTeX
  - Shared counter ensures consistent cross-referencing across all theorem-like environments
  - Changes improve clarity for AI assistants creating problem sets
- **References**: N/A
- **Acknowledgements**: Based on existing problem format used in chapters 1-20
- **Glossary**: No glossary changes
