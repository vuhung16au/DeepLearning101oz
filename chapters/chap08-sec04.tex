...
\begin{tikzpicture}[scale=1.0]
  ...
  \foreach \a/\b in {2.2/1.3, 1.8/1.05, 1.4/0.85, 1.0/0.65, 0.6/0.45} {
    \draw[gray!60] (0,0) ellipse ({\a} and {\b});
  }
  % Current point
  \fill[blue] (-1.2,0.7) circle (1.5pt);
  % Gradient step (steepest descent, unscaled)
  \draw[->, thick, blue!60] (-1.2,0.7) -- ++(0.35,-0.85) node[below right, xshift=1pt, yshift=-2pt] {\small gradient step};
  % Newton step (curvature scaled)
  \draw[->, thick, red!70] (-1.2,0.7) -- ++(0.95,-0.35) node[right, xshift=2pt] {\small Newton step};
  % Optimum
  \fill[black] (0,0) circle (1.2pt) node[below right] {\small $\theta^*$};
\end{tikzpicture}
...
\begin{tikzpicture}[scale=1.0]
  ...
  \foreach \a/\b in {2.2/0.9, 1.8/0.75, 1.4/0.62, 1.0/0.5, 0.6/0.38} {
    \draw[gray!60, rotate=25] (0,0) ellipse ({\a} and {\b});
  }
  % Current point
  \fill[blue] (-1.0,0.3) circle (1.5pt);
  % Euclidean gradient step (blue)
  \draw[->, thick, blue!70] (-1.0,0.3) -- ++(0.25,-0.55) node[below right, xshift=1pt, yshift=-1pt] {\small Euclidean};
  % Natural gradient step (green), rotates/scales toward true steepest path under metric
  \draw[->, thick, green!60!black] (-1.0,0.3) -- ++(0.6,-0.25) node[right, xshift=2pt] {\small Natural};
  % Optimum
  \fill[black] (0,0) circle (1.2pt) node[below right] {\small $\theta^*$};
\end{tikzpicture}
